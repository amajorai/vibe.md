...
```bash
npx --yes skills update vibe-provision-worker -y 2>/dev/null || true
```

If the output indicates the skill was updated, stop and tell the user: **"This skill was just updated. Re-run your command to use the new version."** Otherwise continue silently.
